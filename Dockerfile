...
ENV SNELL_URI="https://github.com/surge-networks/snell/releases/download/2.0.0b8/snell-server-v2.0.0-b8-linux-amd64.zip"
...
RUN apk add --update libstdc++ && rm -rf /var/cache/apk/* && \
    ...
    wget ${SNELL_URI} && \
    unzip snell-server-v2.0.0-b8-linux-amd64.zip && \
    # mv snell-server /usr/bin/ && rm snell-server-v${SNELL_VERSION}-linux-amd64.zip
    mv snell-server /usr/bin/ && rm snell-server-v2.0.0-b8-linux-amd64.zip
